  ...
  return (
    ...
    >
      <div className="mx-4 mt-4">
        <motion.div 
          className="max-w-6xl mx-auto glass-card px-6 py-3"
            ...
            <nav className="hidden md:flex items-center gap-1">
              {navItems.map((item, index) => (
                ...
                >
                  <Link
                    href={item.href}
                    className="px-4 py-2 text-sm text-gray-400 hover:text-white transition-colors rounded-lg hover:bg-white/5 relative group"
                  >
                    {item.name}
                    <motion.span
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              {user ? (
                <div className="relative">
                  ...
                  >
                    <div className="w-8 h-8 rounded-lg bg-gradient-primary flex items-center justify-center text-white font-medium text-sm overflow-hidden">
                      {user.avatar ? (
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-xs text-gray-500">₺{user.balance?.toFixed(2) || '0.00'}</p>
                    </div>
                  </motion.button>
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    onClick={() => setShowUserMenu(!showUserMenu)}
                    className="sm:hidden flex items-center justify-center w-8 h-8 rounded-lg bg-gradient-primary text-white font-medium text-sm overflow-hidden"
                  >
                    {user.avatar ? (
                      <img src={user.avatar} alt={user.name} className="w-full h-full object-cover" />
                    ) : (
                ...
                <>
                  ...
                  <Link href="/register">
                    <motion.button 
                      ...
                      whileTap={{ scale: 0.95 }}
                      className="hidden sm:block px-5 py-2.5 text-sm font-medium text-white btn-primary rounded-xl transition-all shadow-primary"
                    >
                      Kayıt Ol
                    </motion.button>
                  </Link>
                  ...
                  {user ? (
                    ...
                    <>
                      ...
                      <Link href="/register" onClick={() => setIsOpen(false)}>
                        <div className="w-full px-4 py-3 text-white btn-primary rounded-xl font-medium text-center">
                          Kayıt Ol
                        </div>
                      </Link>
                    </>
