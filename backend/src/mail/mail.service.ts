...
export class MailService {
  ...
  async sendMail(to: string, subject: string, html: string) {
    // SMTP ayarları yoksa sessizce atla
  ...
  async sendOrderConfirmation(user: { email: string; name: string }, order: { orderNumber: string; amount: number; product: { name: string } }) {
  ...
  async sendTicketCreated(user: { email: string; name: string }, ticket: { id: number; subject: string }) {
  ...
  async sendTicketReply(user: { email: string; name: string }, ticket: { id: number; subject: string }, replyMessage: string) {
  ...
  async sendTestEmail(to: string) {
    ...
    // Test için SMTP ayarları zorunlu
    if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
  ...
  async sendEmailVerification(user: { email: string; name: string }, verificationToken: string) {
