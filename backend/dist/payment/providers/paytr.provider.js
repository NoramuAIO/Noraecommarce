...
let PaytrProvider = class PaytrProvider {
    ...
    async createPayment(data) {
        ...
        if (!merchantId || !merchantKey || !merchantSalt) {
            throw new common_1.BadRequestException('PayTR ayarları yapılandırılmamış');
        }
        ...
        if (result.status === 'success') {
            ...
            throw new common_1.BadRequestException(`PayTR hatası: ${result.reason}`);
        }
    }
    async handleCallback(body) {
        ...
        if (hash !== expectedHash) {
            throw new common_1.BadRequestException('Geçersiz hash');
        }
